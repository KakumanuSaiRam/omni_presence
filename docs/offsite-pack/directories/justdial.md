# Justdial — paste-ready listing

Claim or create at: https://www.justdial.com/Free-Listing  
Manage afterwards in the JD Business app / Edit Listing.

Search first for existing Guntur listings under “Siva Tuitions” before creating a duplicate.

**Location (locked):** one classroom only — 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006. SVN Colony / AT Agraharam rows on the web are stale; rewrite them to this address. Do not add a branch listing.

---

## Canonical NAP (paste character-for-character)

| Field | Value |
|---|---|
| Business name | Siva Tuitions & Coachings |
| Building / street | 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram |
| Locality | Pattabhipuram |
| City | Guntur |
| State | Andhra Pradesh |
| PIN | 522006 |
| Landmark | behind Jamili Dental Hospital |
| Phone (primary) | +91 88797 97777 |
| WhatsApp | +91 88797 97777 |
| Email | sivatuitions@gmail.com |
| Website | https://sivatuitions.com |
| Contact person | Yadlapalli Naga Murali Krishna |

Do not keyword-stuff the name (no “Best EAPCET Coaching Guntur”). Justdial’s name field must match the signboard.

---

## Categories (Justdial NCT taxonomy)

Pick these from Justdial’s keyword / “Also Listed In” picker. Labels and NCT codes are live Guntur category pages as of 2026.

**Primary (list the business in this first):**

- Tutorials — `nct-10502492`  
  https://www.justdial.com/Guntur/Tutorials/nct-10502492

**Also listed in (add all that apply; blocked categories marked `*` need JD moderation):**

- Tutorials For Eamcet Engineering — `nct-10932905`  
  https://www.justdial.com/Guntur/Tutorials-For-Eamcet-Engineering/nct-10932905
- Neet Tutorials — `nct-11037560`  
  https://www.justdial.com/Guntur/Neet-Tutorials/nct-11037560
- CBSE Tutorials — `nct-10966985`  
  https://www.justdial.com/Guntur/CBSE-Tutorials/nct-10966985
- Tutorials For JEE — `nct-10502848` *(optional; only if you actually run JEE batches. Skip if not.)*

If the picker offers these additional labels, add them:

- Educational Consultants
- Polytechnic Coaching / Diploma CET Coaching
- Intermediate Tuitions
- Home Tutors *(only if you still take home visits — otherwise skip)*

---

## Opening hours

Source: `src/data/site.js` (`Mo-Sa 06:00-21:00`). Confirm before going live.

| Day | Hours |
|---|---|
| Monday–Saturday | 06:00 AM – 09:00 PM |
| Sunday | Closed — `TODO(owner: confirm Sunday hours)` |

Established year: `TODO(owner: confirm; site currently lists 2005)`

---

## Description

Justdial does not publish a hard character limit. Paste the **long** version into About / Business description. Keep the **short** version for any 400-character summary field or JD’s AI-generated 20-word blurb override.

### Short (400 characters max) — 354 chars

```
Siva Tuitions & Coachings, Pattabhipuram, Guntur — tuitions for Classes 1–10 (CBSE, ICSE, AP State) and Intermediate MPC & BiPC, plus coaching for AP EAPCET (EAMCET), NEET and POLYCET. Admission counselling covers web options, certificates and college choice. Director: Yadlapalli Naga Murali Krishna (M.B.A, M.Tech CSE, M.Sc, MCA). Call +91 88797 97777.
```

### Long (about 1,000 characters) — 958 chars

```
Siva Tuitions & Coachings is a tuition and entrance-coaching institute in Pattabhipuram, Guntur. We teach Classes 1–10 across CBSE, ICSE and AP State syllabus, Intermediate MPC and BiPC, and run regular plus short-term crash batches for AP EAPCET (EAMCET), NEET UG, POLYCET and pharmacy-stream entrances.

What students and parents usually need after the exam is as important as the coaching itself. Our admission-counselling desk — led by director Yadlapalli Naga Murali Krishna (M.B.A, M.Tech CSE, M.Sc, MCA) — handles rank analysis, certificate-verification checklists and web-options entry for EAPCET, NEET, POLYCET, ICET, ECET, PG-CET and JoSAA. Families from Guntur city, Guntur district, Andhra Pradesh and Telangana can visit in person or take a phone / WhatsApp session.

Address: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006. Phone / WhatsApp: +91 88797 97777. Website: https://sivatuitions.com
```

---

## Services list (add as separate JD services)

1. Class 1–10 tuitions (CBSE, ICSE, AP State)
2. Intermediate MPC tuitions (Maths, Physics, Chemistry)
3. Intermediate BiPC tuitions (Botany, Zoology, Physics, Chemistry)
4. AP EAPCET / EAMCET coaching (MPC engineering stream)
5. AP EAPCET coaching (BiPC pharmacy / agriculture stream)
6. NEET UG coaching
7. POLYCET / polytechnic coaching (men and women)
8. Short-term crash coaching (EAPCET, POLYCET, pharmacy)
9. EAPCET web-options and admission counselling
10. NEET / medical admission counselling
11. After-tenth counselling (Inter vs Polytechnic)
12. Graduate counselling — ICET, ECET, PG-CET, JoSAA

---

## Keywords (JD “Also Listed In” / search keywords)

EAPCET coaching Guntur, EAMCET coaching Guntur, NEET coaching Guntur, POLYCET coaching Guntur, intermediate tuitions Guntur, tenth class tuitions Pattabhipuram, web options counselling Guntur, educational consultant Guntur, tuition point Pattabhipuram, pharmacy entrance coaching Guntur

Do not paste these into the **business name**.

---

## Photos checklist (JD recommends 10+)

- [ ] Exterior with the board “Siva Tuitions & Coachings”
- [ ] Street approach / 2nd Lane, landmark towards Jamili Dental Hospital
- [ ] Classroom (wide shot, no identifiable minors without parent consent)
- [ ] Second classroom / study hall
- [ ] Counselling desk
- [ ] Director portrait — Yadlapalli Naga Murali Krishna
- [ ] Reception / notice board with batch timings
- [ ] Logo (square crop)
- [ ] Website or app screenshot (optional)
- [ ] `TODO(owner: supply originals; do not scrape other institutes’ photos)`

---

## Other JD fields

| Field | Paste |
|---|---|
| Year established | `TODO(owner: confirm; 2005 is listed on the website pending verification)` |
| Modes of payment | Cash, UPI — `TODO(owner: add cards if you accept them)` |
| Parking | `TODO(owner: street / none)` |
| Women-friendly / family | Yes (co-ed batches; POLYCET guidance includes government polytechnics for women) |
| Online classes | `TODO(owner: yes/no — UrbanPro currently shows Zoom; confirm)` |

---

## Gotchas

- Search Justdial for “Siva Tuitions Guntur” **before** creating. Duplicate listings with SVN Colony or AT Agraharam addresses already exist on other directories; JD may have a scraped copy. Claim the existing one and rewrite NAP to **2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram**. Do not keep SVN Colony or AT Agraharam as a second Justdial listing — the institute is only at Pattabhipuram 2nd Lane.
- Do not put “Best” or “No.1” in the name. That is a common JD/Google suspension pattern.
- Category `Tutorials` is the parent bucket Guntur parents actually search. Entrance categories are extra visibility, not a replacement.
- Phone must stay +91 88797 97777. `TODO(owner: verify this number before first publish — site.js still flags it.)`
- Reply to every JD review as the owner. Never buy ratings.
