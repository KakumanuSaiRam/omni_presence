# Bing Places for Business — paste-ready listing

Portal: https://www.bingplaces.com  
Sign in with a Microsoft account (the owner’s, not a shared student login).

ChatGPT and Copilot retrieve local businesses heavily from **Bing**. This listing is the Phase 3 item most Guntur institutes skip.

Fastest path: **Import from Google** after GBP is verified, then **re-check every field** against this pack. Bing categories do not map 1:1 from Google.

**Location (locked):** 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram. If Bing imported an SVN Colony or Gujjanagundla pin, move it. One listing only.

---

## Canonical NAP (paste character-for-character)

| Field | Value |
|---|---|
| Business name | Siva Tuitions & Coachings |
| Country | India |
| Street address | 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram |
| City | Guntur |
| State / region | Andhra Pradesh |
| Postal code | 522006 |
| Phone | +91 88797 97777 |
| Email | sivatuitions@gmail.com |
| Website | https://sivatuitions.com |
| Opening date | `TODO(owner: confirm; website lists 2005)` |

Name must match GBP exactly. No “Siva Tuitions Guntur EAPCET” modifier.

---

## Categories (Bing Places)

Bing asks for a **business segment** first, then up to **10 categories**, with one marked primary.

**Segment:** Education *(if the dropdown shows Education / Educational Services)*

**Primary category** (pick the most specific of these that the picker actually offers):

1. Tutoring Services  
2. Educational Services  
3. Coaching Center / Coaching Centre  
4. Private Tutor *(only if Tutoring Services is missing)*

**Additional categories (tick only if present and true):**

- Educational Consultant
- Education Center
- Test Preparation Center
- Private School *(usually **no** — you are not a school)*
- Community School *(no)*

Do not select College or University.

If Bing’s picker after Google-import shows only “School”, change it to Tutoring / Educational Services before publish.

---

## Opening hours

| Day | Hours |
|---|---|
| Monday | 06:00 – 21:00 |
| Tuesday | 06:00 – 21:00 |
| Wednesday | 06:00 – 21:00 |
| Thursday | 06:00 – 21:00 |
| Friday | 06:00 – 21:00 |
| Saturday | 06:00 – 21:00 |
| Sunday | Closed — `TODO(owner: confirm)` |

Bing publishes no official description character limit. Whitespark and Bing Places help treat it as a factual summary. Target **150–300 words** (this draft is 150 words / 1,054 characters).

---

## Description (paste)

```
Siva Tuitions & Coachings is a tuition and entrance-coaching institute in Pattabhipuram, Guntur, Andhra Pradesh, India. The centre teaches Classes 1–10 across CBSE, ICSE and AP State syllabus, and Intermediate MPC and BiPC, alongside entrance coaching for AP EAPCET (EAMCET), NEET UG and POLYCET. Short-term crash batches run in the weeks before those exams.

The institute also operates an admission-counselling desk. Director Yadlapalli Naga Murali Krishna — M.B.A, M.Tech (CSE), M.Sc, MCA — reviews rank, category and local-area status, then guides certificate verification and web-options entry for AP EAPCET, TS EAMCET, NEET, POLYCET, ICET, ECET, PG-CET and JoSAA. The desk serves families in Guntur city, Guntur district, elsewhere in Andhra Pradesh, and Telangana, in person or by phone and WhatsApp.

Street address: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006.
Phone: +91 88797 97777
Email: sivatuitions@gmail.com
Website: https://sivatuitions.com
Hours: Monday to Saturday, 06:00 to 21:00.
```

Character count of the block above: **1,054** (spaces included). First ~200 characters (what Bing Announcements / some SERP snippets keep):

```
Siva Tuitions & Coachings is a tuition and entrance-coaching institute in Pattabhipuram, Guntur, Andhra Pradesh, India. The centre teaches Classes 1–10 across CBSE, ICSE and AP State syllabus, and Interme
```

Lead is already NAP-relevant; no rewrite needed.

---

## Services

If Bing shows a Services field, add:

- Class 1–10 tuitions (CBSE, ICSE, State)
- Intermediate MPC & BiPC tuitions
- AP EAPCET / EAMCET coaching
- NEET UG coaching
- POLYCET coaching
- Crash courses
- Admission / web-options counselling
- After-tenth counselling
- ICET / ECET / PG-CET / JoSAA guidance

**Service areas** (do not hide the address — this is a premises business): Guntur, Guntur district, Andhra Pradesh, Telangana. Phone counselling is extra, not a reason to mark the listing as service-area-only.

---

## Keywords

Bing has no separate keyword field. Put natural phrases only in the description (already included): EAPCET, EAMCET, NEET, POLYCET, Pattabhipuram, Guntur, web options.

---

## Photos checklist

Bing: up to 100 photos, min about 480×360, max 10 MB.

- [ ] Storefront
- [ ] Classroom (2–3)
- [ ] Director
- [ ] Counselling desk
- [ ] Logo / interior signage
- [ ] Map-friendly exterior so Bing Maps Street-level pin is obvious
- [ ] Tag any “related images” Bing pulls from the web if they show the wrong building

---

## Q&A (fill Bing’s business FAQ if offered)

Q: Where is Siva Tuitions & Coachings?  
A: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006. Phone +91 88797 97777.

Q: Do you offer EAPCET web-options counselling?  
A: Yes. The counselling desk guides rank analysis, certificates and web options for AP EAPCET and related admissions.

---

## Announcements (optional, first 200 characters display)

Reuse GBP weekly posts; trim to 200 characters + link https://sivatuitions.com

---

## Gotchas

- Import-from-Google is fine for speed; **re-select categories**. “Coaching center” on Google may land on a vague Bing school category.
- Verification can take 7–14 days after publish. Do not create a second Places listing while waiting.
- Social links: add Facebook / Instagram / X only if those profiles already show the same NAP.
- Phone: use the same `+91 88797 97777` string as GBP. `TODO(owner: verify number before first Bing publish.)`
- If Bing Maps already dropped a pin on **SVN Colony** or **Gujjanagundla**, claim that place and **move** it to Pattabhipuram 2nd Lane. Those are stale pins, not branches. Two Bing pins with one phone number split ChatGPT citations.
- Do not mark Sunday open unless the owner confirms.
