# AskLaila — paste-ready listing

Add / claim: https://www.asklaila.com (Add a Business / Claim Ownership on an existing card).

**Location (locked):** one classroom only — 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram. If a crowdsourced card already shows SVN Colony or another Guntur locality, claim it and move it here. Do not add a second AskLaila listing.

AskLaila Guntur education listings sit under **Educational Institutions → Training**. Live category labels from AskLaila Guntur pages:

- Entrance Exam Training Institutes  
  https://www.asklaila.com/search/Guntur/arundelpet/entrance-exam-training-institutes/
- Tuitions  
  (example: https://www.asklaila.com/listing/Guntur/brodipet/asr-institute-of-mathematics/Ra6U0WOO/)

---

## Canonical NAP (paste character-for-character)

| Field | Value |
|---|---|
| Business name | Siva Tuitions & Coachings |
| House / street | 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram |
| Locality | Pattabhipuram |
| City | Guntur |
| PIN | 522006 |
| Phone | +91 88797 97777 |
| Email | sivatuitions@gmail.com |
| Website | https://sivatuitions.com |

If the form splits “opposite / landmark”: **behind Jamili Dental Hospital**.

---

## Categories

**Primary:** Entrance Exam Training Institutes  

**Secondary (if the form allows a second category):** Tuitions  

Breadcrumb to aim for:

`Guntur > Educational Institutions > Training > Entrance Exam Training Institutes`

Do not list under Vocational Training Centers or overseas-education consultancies.

---

## Opening hours

Monday–Saturday: 06:00 AM – 09:00 PM  
Sunday: Closed — `TODO(owner: confirm)`

---

## Description

AskLaila cards are short. Prefer the short description; use long only if a textarea appears.

### Short (~400 characters) — 386 chars

```
Siva Tuitions & Coachings, Pattabhipuram, Guntur: Classes 1–10 and Intermediate tuitions, plus AP EAPCET (EAMCET), NEET and POLYCET coaching. Admission counselling for web options and certificates. Director Yadlapalli Naga Murali Krishna (M.B.A, M.Tech CSE, M.Sc, MCA). 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006. +91 88797 97777.
```

### Long (if accepted) — 623 chars

```
Siva Tuitions & Coachings is a coaching and tuition institute in Pattabhipuram, Guntur. We teach Classes 1–10 (CBSE, ICSE, AP State), Intermediate MPC & BiPC, and entrance batches for AP EAPCET / EAMCET, NEET UG and POLYCET.

The counselling desk, led by director Yadlapalli Naga Murali Krishna (M.B.A, M.Tech CSE, M.Sc, MCA), guides EAPCET web options, NEET and POLYCET admissions, and graduate counselling (ICET, ECET, PG-CET, JoSAA) for students from Guntur, Andhra Pradesh and Telangana.

Address: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006
Phone: +91 88797 97777
```

---

## Services (AskLaila “Services” chips)

Peer listings use chips such as `Test: …` and `Other Services: …`. Paste:

**Tests / exams**

- EAPCET / EAMCET
- NEET
- POLYCET
- ICET
- ECET
- PG-CET
- JoSAA (guidance, not a test we conduct)

**Other services**

- School tuitions (Classes 1–10)
- Intermediate tuitions (MPC & BiPC)
- Web options / admission counselling
- After-tenth counselling
- Short-term crash coaching

**Level (if asked):** School, College (Intermediate), Entrance

**Subject (if asked):** Mathematics, Physics, Chemistry, Biology, Botany, Zoology

---

## Keywords

EAPCET coaching Guntur, EAMCET coaching, NEET coaching Guntur, POLYCET, tuitions Pattabhipuram, web counselling Guntur

---

## Photos checklist

AskLaila listings are often photo-poor; even 3 images help.

- [ ] Facade / board
- [ ] Classroom
- [ ] Director
- [ ] `TODO(owner)`

---

## Gotchas

- Search AskLaila for “Siva Tuitions Guntur” before adding. Crowdsourced cards sometimes already exist with a stale locality (SVN Colony, AT Agraharam, Arundelpet). Claim and move them to **Pattabhipuram** — 2nd Lane, behind Jamili Dental Hospital. Do not keep extra cards.
- Locality must be **Pattabhipuram**, not Arundelpet / Brodipet (those are just popular AskLaila education hubs).
- AskLaila displays a “100%” rate-now widget that is not a real rating. Do not screenshot it as social proof.
- Phone format: include `+91 88797 97777` so it matches GBP. If the form strips `+91`, enter `8879797777` and confirm the public page still shows the canonical spacing where possible.
- Website field: `https://sivatuitions.com` (with https).
