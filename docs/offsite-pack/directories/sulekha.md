# Sulekha — paste-ready listing

Create / claim at: https://www.sulekha.com (List your business).  
Existing listing to claim first:

https://www.sulekha.com/profile/siva-tuitions-at-agraharam-guntur

That page currently shows a **different address** (SVN Colony / AT Agraharam). Do not create a second profile until the owner decides branch vs correction.

---

## Canonical NAP (paste character-for-character)

| Field | Value |
|---|---|
| Business / institute name | Siva Tuitions & Coachings |
| Address line | 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram |
| Area / locality | Pattabhipuram |
| City | Guntur |
| State | Andhra Pradesh |
| PIN | 522006 |
| Phone | +91 88797 97777 |
| Email | sivatuitions@gmail.com |
| Website | https://sivatuitions.com |
| Contact person | Yadlapalli Naga Murali Krishna |

---

## Categories (Sulekha live taxonomy)

Sulekha’s Guntur education tree uses these exact labels (from sulekha.com coaching category pages):

**Primary category**

- Entrance Exam Coaching → Engineering EAMCET Coaching

**Add these sub-categories / services (tick all that you actually run)**

- Medical EAMCET Coaching
- NEET Coaching
- Diploma CET Coaching / Polytechnic coaching
- ECET Coaching
- ICET Coaching
- School Tuitions
- Class I–V Tuitions
- Class VI–VIII Tuitions
- Class IX–X Tuitions
- Home Tuitions for 11th & 12th Classes — Physics, Chemistry, Mathematics, Biology / Zoology / Botany *(institute batches, not necessarily home visits)*
- Competitive Exam Coaching
- Distance Education *(only if you still offer distance mode — UrbanPro currently claims this; `TODO(owner: confirm)`)*

Do **not** tick study-abroad, IELTS, CAT, or IAS unless you actually offer them. Sulekha will send those leads and they waste money on pay-per-lead.

---

## Opening hours

| Day | Hours |
|---|---|
| Monday–Saturday | 06:00 AM – 09:00 PM |
| Sunday | Closed — `TODO(owner: confirm)` |

---

## Description

Sulekha asks for a “short yet compelling” business description. Two lengths below.

### Short (~500 characters) — 514 chars

```
Siva Tuitions & Coachings is a Pattabhipuram, Guntur institute for school tuitions, Intermediate MPC & BiPC, and entrance coaching for AP EAPCET (EAMCET), NEET and POLYCET. Director Yadlapalli Naga Murali Krishna (M.B.A, M.Tech CSE, M.Sc, MCA) also runs the admission-counselling desk — rank analysis, certificates and web options — for families in Guntur, Andhra Pradesh and Telangana. Visit 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006. Call +91 88797 97777.
```

### Long (profile About) — 855 chars

```
Siva Tuitions & Coachings teaches Classes 1–10 (CBSE, ICSE, AP State board), Intermediate MPC and BiPC, and coaches students for AP EAPCET / EAMCET, NEET UG and POLYCET, including short-term crash batches before the exam.

The same desk that coaches also guides admissions: EAPCET and TS EAMCET web options, NEET state-quota and AIQ paperwork, POLYCET college lists (including government polytechnics for women), plus ICET, ECET, PG-CET and JoSAA for graduates. Counselling is available in person at Pattabhipuram and over phone or WhatsApp for families who cannot travel.

Director: Yadlapalli Naga Murali Krishna, M.B.A, M.Tech (CSE), M.Sc, MCA.

Address: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006.
Phone / WhatsApp: +91 88797 97777
Email: sivatuitions@gmail.com
Website: https://sivatuitions.com
```

Do not paste the existing Sulekha boilerplate (“proven track record”, “growing number of satisfied students”). It is generic and unverifiable.

---

## Services list (Sulekha “Education & Exam Support”)

- School tuitions — Classes 1–10, CBSE / ICSE / State
- Intermediate tuitions — MPC and BiPC
- Engineering EAMCET / EAPCET coaching
- Medical EAMCET / pharmacy-stream EAPCET coaching
- NEET UG coaching
- POLYCET / diploma CET coaching
- Short-term crash coaching
- Admission / web-options counselling
- After-tenth career counselling

---

## Keywords

EAPCET coaching Guntur, EAMCET coaching Guntur, NEET coaching Guntur, POLYCET Guntur, intermediate tuition Pattabhipuram, web counselling Guntur, tenth class tuition Guntur

---

## Photos checklist

Sulekha states photos can lift profile completeness ~25%.

- [ ] Board / facade
- [ ] Classroom
- [ ] Director
- [ ] Counselling desk
- [ ] Locality photo (Pattabhipuram, not SVN Colony)
- [ ] Logo
- [ ] `TODO(owner: supply; replace any stock images currently on the AT Agraharam profile)`

---

## Other fields

| Field | Paste |
|---|---|
| Mode of training | Classroom; phone/WhatsApp counselling. Online: `TODO(owner)` |
| Gender | Co-ed |
| Experience | `TODO(owner: years since founding)` |
| KYC / ID | Director Aadhaar + institute photo — needed if you want Sulekha verified leads |
| Paid leads | Optional. Free listing is enough for citations. Do not turn on pay-per-lead for irrelevant categories. |

---

## Gotchas

- **Claim https://www.sulekha.com/profile/siva-tuitions-at-agraharam-guntur first.** It currently lists: “BEHIND RVR & JC WOMEN'S HOSTEL … 5th LANE, SVN COLONY, AT Agraharam, Guntur - 522006”. That is **not** the canonical NAP.
- `TODO(owner: branch vs correction)`  
  - If SVN Colony / AT Agraharam was an old centre or a geocoder error → edit that listing to the Pattabhipuram NAP and rename to **Siva Tuitions & Coachings**.  
  - If it is a real second classroom → keep it as a **branch** listing with its own real address, and create/claim a **separate** Pattabhipuram listing with the canonical NAP. Never run two listings with the same phone and two different addresses unless both places are real.
- Sulekha’s current About text claims a “proven track record” with no numbers. Replace it with the copy above. Do not invent student counts to fill the gap.
- Ignore Sulekha’s “Distance Education” and “IT training” ticks unless they are real services.
