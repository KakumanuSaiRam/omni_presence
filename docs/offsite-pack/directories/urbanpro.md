# UrbanPro — paste-ready listing

Existing institute profile (update this; do not spawn a duplicate):

https://www.urbanpro.com/guntur/siva-tuitions-gujjanagundla/31503988

Current address on that page: **Beside HDFC Bank, Stambhalagaruvu, Gujjanagundla, Guntur, India - 522006**.  
Canonical centre is Pattabhipuram. Decide branch vs correction before touching anything else.

---

## Canonical NAP (paste character-for-character)

| Field | Value |
|---|---|
| Institute name | Siva Tuitions & Coachings |
| Address | 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram |
| Locality (if picker forces an area) | Pattabhipuram *(preferred)* / Gang Colony |
| City | Guntur |
| PIN | 522006 |
| Phone | +91 88797 97777 |
| Email | sivatuitions@gmail.com |
| Website | https://sivatuitions.com |
| Contact / director | Yadlapalli Naga Murali Krishna |

---

## Profile type

Choose **Institute** (not individual tutor). The Gujjanagundla page is already an institute profile named “Siva Tuitions”. Rename to the canonical **Siva Tuitions & Coachings**.

Do not merge with unrelated UrbanPro tutor profiles named “Siva” in New Guntur or Dachepalle — those are different people.

---

## Categories (UrbanPro class taxonomy)

UrbanPro matches leads from the classes you tick. Tick only what you teach. The live Gujjanagundla profile currently includes **ITIL V3 Intermediate Training** and **BTech / AI** — those look like leftover or mismatched ticks. Turn them **off** unless they are real.

**Keep / add:**

| UrbanPro class | Details to fill |
|---|---|
| Class I-V Tuition | Boards: CBSE, ICSE, State. Subjects: Maths, Science, English, Telugu, Social Science, Computers. Location: At the Institute. |
| Class VI-VIII Tuition | Same boards. Add if not already listed. |
| Class IX-X Tuition / Class 10 Tuition | SSC / CBSE / ICSE. All core subjects. |
| Class 11 Tuition | Boards: State, CBSE, ISC/ICSE. MPC + BiPC subjects. |
| Class 12 Tuition | Same. |
| Engineering Entrance Coaching classes | Exams: **EAMCET** (this is UrbanPro’s label for EAPCET/EAMCET). Type: Regular Classes, Crash Course. Location: At the Institute. |
| NEET-UG Coaching Classes | Regular + crash. Biology, Physics, Chemistry. |
| Medical Entrance Coaching classes | Only as a secondary if NEET-UG is listed; do not add AIIMS/AIPMT as if you run those dedicated batches. |
| Polytechnic / diploma coaching | If the picker has POLYCET / Diploma CET, add it. Otherwise describe POLYCET under Engineering Entrance + Class 10. |

**Turn off unless owner confirms they are real services:**

- ITIL V3 Intermediate Training *(currently on the Gujjanagundla profile)*
- BTech Computer Science — Artificial Intelligence
- Software Training Institutes
- Home tuition at student’s home *(tick only if you still travel)*

Class location defaults: **At the Institute**. Online via Zoom: `TODO(owner: the current profile advertises Zoom — confirm)`.

---

## Tagline (UrbanPro “profile tagline”, keep short)

```
Tuitions, EAPCET/NEET/POLYCET coaching & web-options counselling — Pattabhipuram, Guntur
```

(118 characters. If the field caps near 80, use:)

```
EAPCET, NEET, POLYCET coaching & tuitions — Pattabhipuram, Guntur
```

---

## About / description

UrbanPro bios are free-text; keep this under ~1,200 characters so it does not look like spam.

**Paste (747 chars):**

```
Siva Tuitions & Coachings is an institute in Pattabhipuram, Guntur. We teach Classes 1–10 (CBSE, ICSE, AP State), Intermediate MPC and BiPC, and we coach for AP EAPCET (EAMCET), NEET UG and POLYCET, including short-term crash batches.

Director Yadlapalli Naga Murali Krishna (M.B.A, M.Tech CSE, M.Sc, MCA) also runs the admission-counselling desk: rank analysis, certificate checklists and web options for EAPCET, NEET, POLYCET, ICET, ECET, PG-CET and JoSAA. Students from Guntur city, Guntur district, Andhra Pradesh and Telangana can visit or take a phone / WhatsApp session.

Address: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006.
Phone / WhatsApp: +91 88797 97777
https://sivatuitions.com
```

Replace the current one-line dump (“Inter (Maths Physics Chemistry English) 6,7,8,9,10 … Iiit Mains Jee Aprjc Etc. With Distance Mode Also”). That line is unreadable and claims IIIT/JEE/APRJC/distance mode without confirmation.

`TODO(owner: confirm whether distance mode, IIIT, JEE and APRJC batches are real. If not, do not retick them.)`

---

## Opening hours

UrbanPro often infers hours from class slots rather than a GBP-style grid. If a hours field exists:

Monday–Saturday 06:00–21:00. Sunday: `TODO(owner)`.

Experience years: `TODO(owner)` — do not copy “13 years” from unrelated “Siva” tutor profiles.

---

## Services / classes offered (plain list for the “Teaches” editor)

1. Class 1–5 tuition — CBSE, ICSE, State
2. Class 6–8 tuition
3. Class 9–10 / SSC tuition
4. Intermediate 1st & 2nd year MPC
5. Intermediate 1st & 2nd year BiPC
6. AP EAPCET / EAMCET (engineering and pharmacy streams)
7. NEET UG
8. POLYCET
9. Crash courses
10. Admission counselling / web options (describe in About; UrbanPro may not have this as a class)

---

## Keywords

Not a separate UrbanPro field. They come from class names. Accurate class ticks matter more than stuffing the bio.

---

## Photos checklist

UrbanPro requires a profile photo for Tutor Partner eligibility.

- [ ] Institute logo or director headshot (clear, no collage)
- [ ] Classroom
- [ ] Exterior at **Pattabhipuram** (not the Gujjanagundla HDFC landmark unless that site is still real)
- [ ] ID proof for verification (director)

---

## Fees

Leave blank or `TODO(owner: publish only real batch fees)`. UrbanPro leads often ask fees in the first message — keep a private rate card, do not invent numbers here.

---

## Gotchas

- **Update https://www.urbanpro.com/guntur/siva-tuitions-gujjanagundla/31503988.** Creating a second Guntur institute with the same phone will look like a duplicate and split reviews (there are currently none — good time to fix NAP).
- `TODO(owner: branch vs correction)`  
  - If Gujjanagundla / Stambhalagaruvu (Beside HDFC Bank) is **not** a current classroom → change locality to Pattabhipuram and paste the canonical street address.  
  - If it **is** a branch → UrbanPro allows one locality per profile; either keep Gujjanagundla as that branch’s listing and add a **new** institute profile for Pattabhipuram with the canonical NAP, or close Gujjanagundla if that room no longer exists.
- Untick ITIL V3. That category puts you in “Software Training Institutes” carousels, which is how this profile currently appears next to IT institutes — wrong audience.
- Do not buy UrbanPro “Certified” or fake reviews. Certified is awarded from student feedback.
- Disclose paid UrbanPro membership only if you need demo-booking; a corrected free profile is still a useful citation.
