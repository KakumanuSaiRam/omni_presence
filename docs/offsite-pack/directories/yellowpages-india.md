# Yellow Pages India — paste-ready listing

Use the national directory that actually indexes Guntur education listings. Prefer **yellowpages.co.in** / **indianyellowpages** style forms if you already have a login; otherwise use:

- https://www.yellowpages-india.in (Add Business)
- https://www.indiayellowpagesonline.in/addlisting.htm (free company add)

If a sales PDF cites a different “Yellow Pages India” domain, still paste the **same NAP**. The citation value is consistency, not which clone ranks this month.

**Location (locked):** 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram. Claim and rewrite any SVN Colony / AT Agraharam row; those are not branches.

---

## Canonical NAP (paste character-for-character)

| Field | Value |
|---|---|
| Company / business name | Siva Tuitions & Coachings |
| Address | 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram |
| City | Guntur |
| State | Andhra Pradesh |
| PIN | 522006 |
| Phone | +91 88797 97777 |
| Email | sivatuitions@gmail.com |
| Website | https://sivatuitions.github.io *(paste this until sivatuitions.com DNS is live)* |
| Contact person | Yadlapalli Naga Murali Krishna |

---

## Categories (Yellow Pages India taxonomy)

Yellow Pages India / Jim Yellowpages-style trees use these education headings (live FAQ category list includes them):

**Primary**

- Educational Institutions & Services

**Secondary (max often 5 product categories on India Yellow Pages Online)**

- Coaching Classes / Coaching Institutes
- Educational Consultants
- Tuition Classes
- Education & Training

If the form is a free-text “product category” box, paste exactly:

1. Coaching Classes
2. Tuition Classes
3. Educational Consultants
4. Entrance Exam Coaching
5. Educational Institutions & Services

Do not pick “Education (Foreign & Indian) Consultants” — that bucket is study-abroad.

---

## Opening hours

Monday–Saturday: 06:00 AM – 09:00 PM  
Sunday: 10:00 AM – 02:00 PM

---

## Description

India Yellow Pages Online labelled field: **“Describe Your Products / Activity in Brief.”** No public hard cap; keep it under 1,000 characters so clone sites do not truncate mid-sentence.

### Brief (for “activity in brief” boxes) — 469 chars

```
Coaching and tuitions in Pattabhipuram, Guntur: Classes 1–10 (CBSE, ICSE, AP State), Intermediate MPC & BiPC, AP EAPCET (EAMCET), NEET UG and POLYCET, plus admission counselling (web options, certificates) for EAPCET, NEET, POLYCET, ICET, ECET, PG-CET and JoSAA. Director: Yadlapalli Naga Murali Krishna, M.B.A, M.Tech (CSE), M.Sc, MCA. Address: 2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006. Phone: +91 88797 97777.
```

### Full About — 787 chars

```
Siva Tuitions & Coachings is a Guntur institute offering school and Intermediate tuitions together with entrance coaching and a counselling desk.

Tuitions: Classes 1–10 (CBSE, ICSE, AP State) and Intermediate MPC & BiPC.
Entrance coaching: AP EAPCET / EAMCET (engineering and pharmacy streams), NEET UG, POLYCET, and short-term crash batches.
Counselling: rank analysis, certificate verification and web options for EAPCET, NEET, POLYCET, ICET, ECET, PG-CET and JoSAA, for students from Guntur district, Andhra Pradesh and Telangana.

Director Yadlapalli Naga Murali Krishna holds an M.B.A, M.Tech (CSE), M.Sc and MCA.

2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006
+91 88797 97777 · sivatuitions@gmail.com · https://sivatuitions.github.io
```

---

## Services list

- Class 1–10 tuitions
- Intermediate MPC & BiPC tuitions
- EAPCET / EAMCET coaching
- NEET coaching
- POLYCET coaching
- Crash coaching
- Web-options / admission counselling
- After-tenth counselling
- Graduate counselling (ICET, ECET, PG-CET, JoSAA)

---

## Keywords

EAPCET coaching Guntur, EAMCET coaching, NEET coaching, POLYCET, intermediate tuitions, educational consultant Guntur, Pattabhipuram tuitions

---

## Photos checklist

- [ ] Logo
- [ ] Exterior
- [ ] Classroom
- [ ] Director
- [ ] `TODO(owner)`

Many Yellow Pages clones ignore photos; still upload so aggregators that scrape the profile pick up an image.

---

## Other fields

| Field | Paste |
|---|---|
| Year established | 2005 |
| Business type | Service — Education |
| Working days | Monday to Saturday |

---

## Gotchas

- There are several “Yellow Pages India” domains. Submit **once** on the property you can log into, and use this same NAP if a data aggregator (Foursquare, Bing) later asks you to confirm a Yellow Pages-sourced listing.
- Do not pay for “top 10 listing with logo” until the free NAP is live and correct. Paid upgrades do not fix a wrong address.
- If an existing Yellow Pages row shows **SVN Colony** or **AT Agraharam**, claim it and rewrite the address to **2nd Lane, behind Jamili Dental Hospital, Gang Colony, Pattabhipuram, Guntur, Andhra Pradesh 522006**. Those old localities are not branches.
- Never paste competitor names into the description.
